...
colors = [(255 / 255, 236 / 255, 0 / 255), (52 / 255, 211 / 255, 77 / 255), (200 / 255, 250 / 255, 40 / 255),
          ...
          (171 / 255, 37 / 255, 157 / 255), (226 / 255, 0 / 255, 26 / 255), (184 / 255, 241 / 255, 204 / 255),
...
def my_circle_draw(inputfile, short_TR, long_TR, dis_R, outputfile, dis_R_Threshold, interval_ratio, draw_exon=True,
    ...
    if draw_three_repeat and interval_ratio == 1:
        ...
        plt.text(0.52, 0.92, 'Mitochondrial Genome', ha='center', va='center', rotation=0,
        ...
        plt.text(0.52, 0.50, 'Mitochondrial Genome', ha='center', va='center', rotation=0,
            ...
            if gene_name in genes.gene_names and len(genes.genes_info[gene_name]['exon']) > 0 and draw_exon:
                if not genes_flag[gene_name]:
                    for feature in genes.genes_info[gene_name]['exon']:
                        ...
                        if "number" not in feature.qualifiers.keys():
                            raise noExonNumberError(feature)
    ...
    for i in range(len(dis_location)):
        ...
        if draw_three_repeat and interval_ratio == 1:
            if dis_location[i][2] == 1:
                ax.bar(theta1, 7.5, width=ratio_old * dis_location[i][0], bottom=61.5, color='orange', edgecolor='orange',
                       linewidth=1.5, alpha=0.6)
                ax.bar(theta2, 7.5, width=ratio_old * dis_location[i][0], bottom=61.5, color='orange', edgecolor='orange',
                       linewidth=1.5, alpha=0.6)
            if dis_location[i][2] == -1:
    ...
    for i in range(1, len(record.features)):
        ...
        if draw_misc_feature:
            if record.features[i].type == 'misc_feature':
                theta = ratio_old * (int(record.features[i].location.start) + int(record.features[i].location.end)) / 2
                plt.scatter(theta, 200, s=40, edgecolors=colors[13], marker='o', alpha=0.8, linewidths=1.3,
                            facecolors='none')
        if draw_ncRNA:
        ...
        if draw_5UTR:
            if record.features[i].type == "5'UTR":
                theta = ratio_old * (int(record.features[i].location.start) + int(record.features[i].location.end)) / 2
                plt.scatter(theta, 188, s=40, edgecolors=colors[4], marker='P', alpha=0.8, linewidths=1.3,
                            facecolors='none')
        if draw_3UTR:
            if record.features[i].type == "3'UTR":
                theta = ratio_old * (int(record.features[i].location.start) + int(record.features[i].location.end)) / 2
                plt.scatter(theta, 212, s=40, edgecolors='brown', marker='d', alpha=0.8, linewidths=1.3, facecolors='none')

    ax.set_rlim(bottom=0, top=235)
    # ax.set_rmax(np.max(224.5))
    # 添加图例 左边
    ax = fig.add_axes([0.15, 0, 0.65, 0.10],
    ...
    plt.text(0.03, 0.64, 'complex I (NADH dehydrogenase)', ha='left', va='center', rotation=0, wrap=True)
    plt.gca().add_patch(
    ...
    plt.gca().add_patch(plt.Rectangle(xy=(0.001, 0.54), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.03, 0.56, 'complex II (succinate dehydrogenase)', ha='left', va='center', rotation=0, wrap=True)
    plt.gca().add_patch(
    ...
    plt.gca().add_patch(plt.Rectangle(xy=(0.001, 0.46), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.03, 0.48, 'complex III (ubichinol cytochrome c reductase)', ha='left', va='center', rotation=0,
             wrap=True)
    plt.gca().add_patch(
    ...
    plt.gca().add_patch(plt.Rectangle(xy=(0.001, 0.38), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.03, 0.40, 'complex IV (cytochrome c oxidase)', ha='left', va='center', rotation=0, wrap=True)
    plt.gca().add_patch(
    ...
    plt.gca().add_patch(plt.Rectangle(xy=(0.001, 0.30), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.03, 0.32, 'ATP synthase', ha='left', va='center', rotation=0, wrap=True)
    plt.gca().add_patch(
        plt.Rectangle(xy=(0, 0.22), width=0.014, height=0.05, fill=True, color=colors[7]))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.001, 0.22), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.03, 0.24, 'ribosomal proteins(SSU)', ha='left', va='center', rotation=0, wrap=True)
    plt.gca().add_patch(
        plt.Rectangle(xy=(0, 0.14), width=0.014, height=0.05, fill=True, color=colors[8]))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.001, 0.14), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.03, 0.16, 'ribosomal proteins(LSU)', ha='left', va='center', rotation=0, wrap=True)
    plt.gca().add_patch(
        plt.Rectangle(xy=(0, 0.06), width=0.014, height=0.05, fill=True, color=colors[9]))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.001, 0.06), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.03, 0.08, 'maturases', ha='left', va='center', rotation=0, wrap=True)
    plt.text(0, 0.01, 'The colored parabola in the center circle represents the Dispersed Repeats', ha='left',
             va='center', rotation=0, wrap=True)

    # 添加图例 右边
    plt.gca().add_patch(
        plt.Rectangle(xy=(0.33, 0.62), width=0.014, height=0.05, fill=True, color=colors[11]))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.331, 0.62), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.36, 0.64, 'transfer RNAs', ha='left', va='center', rotation=0, wrap=True)
    plt.gca().add_patch(
        plt.Rectangle(xy=(0.33, 0.54), width=0.014, height=0.05, fill=True, color=colors[6]))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.331, 0.54), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.36, 0.56, 'ribosomal RNAs', ha='left', va='center', rotation=0, wrap=True)
    plt.gca().add_patch(
        plt.Rectangle(xy=(0.33, 0.46), width=0.014, height=0.05, fill=True, color='purple'))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.331, 0.46), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.36, 0.48, 'other genes', ha='left', va='center', rotation=0, wrap=True)
    plt.gca().add_patch(
        plt.Rectangle(xy=(0.33, 0.38), width=0.014, height=0.05, fill=True, color='Tomato'))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.331, 0.38), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.36, 0.40, 'Short Tandem Repeats', ha='left', va='center', rotation=0, wrap=True)
    plt.gca().add_patch(
        plt.Rectangle(xy=(0.33, 0.30), width=0.014, height=0.05, fill=True, color='BlueViolet'))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.331, 0.30), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.36, 0.32, 'Long Tandem Repeats', ha='left', va='center', rotation=0, wrap=True)
    plt.gca().add_patch(
        plt.Rectangle(xy=(0.33, 0.22), width=0.014, height=0.05, fill=True, color='orange', alpha=0.6))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.331, 0.22), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.358, 0.22), width=0.014, height=0.05, fill=True, color='LightSalmon',
                                      alpha=0.5))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.358, 0.22), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.386, 0.24, 'Dispersed Repeats(direct matches)', ha='left', va='center', rotation=0, wrap=True)
    plt.gca().add_patch(
        plt.Rectangle(xy=(0.33, 0.14), width=0.014, height=0.05, fill=True, color='green', alpha=0.6))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.331, 0.14), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.358, 0.14), width=0.014, height=0.05, fill=True, color='Aquamarine',
                                      alpha=0.5))  # xy是矩形的左下角坐标
    plt.gca().add_patch(plt.Rectangle(xy=(0.359, 0.14), width=0.014, height=0.05, fill=False, edgecolor='black',
                                      linewidth=0.5))  # xy是矩形的左下角坐标
    plt.text(0.386, 0.16, 'Dispersed Repeats(palindromic matches)', ha='left', va='center', rotation=0, wrap=True)

    plt.scatter(0.56, 0.64, s=60, edgecolors=colors[13], marker='o', alpha=0.8, linewidths=1.3, facecolors='none')
    plt.text(0.57, 0.64, 'misc feature', ha='left', va='center', rotation=0, wrap=True)
    plt.scatter(0.56, 0.56, s=60, edgecolors=(255 / 255, 105 / 255, 180 / 255), marker='*', alpha=0.8, linewidths=1.3,
                facecolors='none')
    plt.text(0.57, 0.56, 'ncRNA', ha='left', va='center', rotation=0, wrap=True)
    plt.scatter(0.56, 0.48, s=60, edgecolors=(102 / 255, 205 / 255, 170 / 255), marker='v', alpha=0.8, linewidths=1.3,
                facecolors='none')
    plt.text(0.57, 0.48, 'repeat region', ha='left', va='center', rotation=0, wrap=True)
    plt.scatter(0.56, 0.40, s=60, edgecolors=colors[4], marker='P', alpha=0.8, linewidths=1.3, facecolors='none')
    plt.text(0.57, 0.40, "5'UTR", ha='left', va='center', rotation=0, wrap=True)
    plt.scatter(0.56, 0.32, s=60, edgecolors='brown', marker='d', alpha=0.8, linewidths=1.3, facecolors='none')
    plt.text(0.57, 0.32, "3'UTR", ha='left', va='center', rotation=0, wrap=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', help='the RNA file to edit', type=str,
                        default='./20230523093041_31_0.gb')
